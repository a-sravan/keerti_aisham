...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  function markdownToSafeHtml(text) {
    // Replace URLs not already in markdown link format with markdown links
    const urlRegex = /(?<!]\()(https?:\/\/[^\s]+)(?!\))/g;
    const textWithLinks = text.replace(urlRegex, '[$1]($1)');
    
    // Convert markdown to HTML
    const rawHtml = marked.parse(textWithLinks, {
      breaks: true,
      gfm: true
    });
  ...
  function addBotMessageWithTyping(text) {
    ...
    botMessage.innerHTML = `<div class="message-content"><div class="markdown-content"></div></div>`;
    chatMessages.appendChild(botMessage);

    const messageDiv = botMessage.querySelector('.markdown-content');
    let index = 0;
    
    function typeWriter() {
      if (index < text.length) {
        const currentText = text.substring(0, index + 1);
        messageDiv.innerHTML = markdownToSafeHtml(currentText);
        index++;
        scrollToBottom();
  ...
  function addBotMessage(text) {
    const botMessage = document.createElement('div');
    botMessage.className = 'message bot';
    botMessage.innerHTML = `<div class="message-content"><div class="markdown-content">${markdownToSafeHtml(text)}</div></div>`;
    chatMessages.appendChild(botMessage);
    scrollToBottom();
  }
